...
static struct device_attribute powermeter_attr[] = {
	__ATTR(adc_val_all, S_IRUGO, adc_val_all_show, NULL),
	__ATTR(adc_en, S_IRUGO | S_IWUSR, adc_en_show, adc_en_store),
	__ATTR(adc_mode, S_IRUGO | S_IWUSR, adc_mode_show, adc_mode_store),
	__ATTR(adc_sync_mode, S_IRUGO | S_IWUSR, adc_sync_mode_show, adc_sync_mode_store),
	__ATTR(adc_val_0, S_IRUGO, adc_val_0_show, NULL),
	__ATTR(adc_val_1, S_IRUGO, adc_val_1_show, NULL),
	__ATTR(adc_val_2, S_IRUGO, adc_val_2_show, NULL),
	__ATTR(adc_val_3, S_IRUGO, adc_val_3_show, NULL),
	__ATTR(adc_val_4, S_IRUGO, adc_val_4_show, NULL),
	__ATTR(adc_val_5, S_IRUGO, adc_val_5_show, NULL),
	__ATTR(adc_val_6, S_IRUGO, adc_val_6_show, NULL),
	__ATTR(adc_val_7, S_IRUGO, adc_val_7_show, NULL),
	__ATTR(adc_ctrl1, S_IRUGO | S_IWUSR, adc_ctrl1_show, adc_ctrl1_store),
#if IS_ENABLED(CONFIG_SEC_PM)
	__ATTR(adc_validity, S_IRUGO, adc_validity_show, NULL),
#endif /* CONFIG_SEC_PM */
};
...
static int s2dos05_create_powermeter_sysfs(struct s2dos05_dev *s2dos05)
{
	...
	s2dos05_adc_dev = sec_device_create(s2dos05->adc_meter, device_name);
	s2dos05->powermeter_dev = s2dos05_adc_dev;

	/* Create sysfs entries */
	for (i = 0; i < ARRAY_SIZE(powermeter_attr); i++) {
		err = device_create_file(s2dos05_adc_dev, &powermeter_attr[i]);
		if (err)
...
remove_pmic_device:
	for (i--; i >= 0; i--)
		device_remove_file(s2dos05_adc_dev, &powermeter_attr[i]);
	sec_device_destroy(s2dos05_adc_dev->devt);
