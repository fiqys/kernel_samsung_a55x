...
static struct device_attribute regulator_attr[] = {
	__ATTR(s2dos05_write, 0644, s2dos05_write_show, s2dos05_write_store),
	__ATTR(s2dos05_read, 0644, s2dos05_read_show, s2dos05_read_store),
};
...
static int s2dos05_create_sysfs(struct s2dos05_data *s2dos05)
{
	...
	s2dos05_pmic = sec_device_create(s2dos05, device_name);
	s2dos05->dev = s2dos05_pmic;

	/* Create sysfs entries */
	for (i = 0; i < ATTR_REGULATOR; i++) {
		err = device_create_file(s2dos05_pmic, &regulator_attr[i]);
		if (err)
			goto remove_sec_device;
	}
...
remove_sec_device:
	for (i--; i >= 0; i--)
		device_remove_file(s2dos05_pmic, &regulator_attr[i]);
	sec_device_destroy(s2dos05_pmic->devt);
