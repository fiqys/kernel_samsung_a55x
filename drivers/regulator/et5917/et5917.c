...
#define ET5917SX_LDO12_VOLTAGES 225
...
#define ET5917SX_LDO37_VOLTAGES 256
...
int et5917sx_recovery_voltage(struct regulator_dev *rdev)
{
	...
	if (rdev->desc->vsel_mask == ET5917SX_LDO_VSEL_MASK && vol != reg)
...
int et5917sx_recovery(struct et5917sx_data *et5917sx)
{
	...
	for (i = 0; i < et5917sx->num_regulators; i++) {
		...
		if (regulator_should_be_enabled(rdev) && !et5917sx_is_enabled_regmap(rdev)) {
...
static int et5917sx_regulator_resume(struct device *dev)
{
	...
	if (et5917sx->need_self_recovery)
					...
					struct et5917sx_platform_data *pdata)
{
	...
	pdata->need_self_recovery = of_property_read_bool(et5917sx_np, "et5917,need_self_recovery");
