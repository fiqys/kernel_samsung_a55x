...
static int verity_handle_err(struct dm_verity *v, struct dm_verity_io *io,
			     ...
			     u8 *dest, struct bvec_iter *iter)
{
	...
	verity_dump_memory(v, io, type, block, dest, iter);
#endif
			       ...
			       u8 *want_digest)
{
	...
	if (!aux->hash_verified) {
		...
		if (likely(memcmp(verity_io_real_digest(v, io), want_digest,
			...
			add_corrupted_blks();
#else
		else if (verity_handle_err(v, io,
					   DM_VERITY_BLOCK_TYPE_METADATA,
					   hash_block, data, NULL)) {
#endif
			r = -EIO;
			goto release_ret_r;
		}
	...
	for (b = 0; b < io->n_blocks; b++) {
		...
		if (likely(memcmp(verity_io_real_digest(v, io),
			...
			if (verity_fec_is_enabled(v)) {
				...
				DMERR_LIMIT("%s: [DMV] iter is NULL", v->data_dev->name);
				iter = NULL;
			}
#ifdef SEC_HEX_DEBUG
...
#else
			if (verity_handle_err(v, io, DM_VERITY_BLOCK_TYPE_DATA,
					      cur_block, NULL, iter))
				return -EIO;
#endif
		}
