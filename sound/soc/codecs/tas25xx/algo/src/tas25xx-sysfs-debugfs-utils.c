...
/* user impedance: (detect_val/2^19): QFORMAT19 */
...
int smartpa_read_freq_dbg(char *buffer, int size)
{
	...
	for (iter = 0; iter < channels; iter++) {
		...
		while ((i[iter]++ < 5) && (j[iter] < 3)) {
			...
			if (((F0[iter] >> QFORMAT19) < s_smartpa_algo->fres_min[iter])
				|| ((F0[iter] >> QFORMAT19) > s_smartpa_algo->fres_max[iter])
				|| (((Q[iter] * 100) >> QFORMAT19) < s_smartpa_algo->Qt[iter]))
				j[iter] = 0;
			else
				j[iter]++;
...
int smartamp_set_i2c_client(struct i2c_client *i2c)
{
